...
const headerPath = path.join(root, "patches", "match-100-user-paste.txt");
const partsDir = path.join(root, "patches");
const outPath = path.join(root, "pastes", "match-100.txt");

const parts = fs
  .readdirSync(partsDir)
  .filter((f) => f.match(/^match-100-listings-part\d+\.txt$/))
...
const listings = (full.match(/GEHA Field at Arrowhead Stadium - Section/g) || []).length;
