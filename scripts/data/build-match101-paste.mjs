...
const headerPath = path.join(root, "patches", "match-101-user-paste.txt");
const partsDir = path.join(root, "patches");
const outPath = path.join(root, "pastes", "match-101.txt");

const parts = fs
  .readdirSync(partsDir)
  .filter((f) => f.match(/^match-101-listings-part\d+\.txt$/))
...
const listings = (full.match(/AT&T Stadium - Section/g) || []).length;
