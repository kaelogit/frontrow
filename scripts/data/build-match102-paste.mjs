...
const headerPath = path.join(root, "patches", "match-102-user-paste.txt");
const partsDir = path.join(root, "patches");
const outPath = path.join(root, "pastes", "match-102.txt");

const parts = fs
  .readdirSync(partsDir)
  .filter((f) => f.match(/^match-102-listings-part\d+\.txt$/))
...
const listings = (full.match(/Mercedes-Benz Stadium - Section/g) || []).length;
